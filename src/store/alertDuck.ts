...
const reducer =
	reducerWithInitialState<AlertState>(initialState)
	.case(actions.showSuccsessMessage, (state, payload) => ({ ...state, text: payload}))
	.case(actions.hideSuccessMessage, state => ({ ...state, text: ''}))
	.case(actions.showErrorMessage, state => ({ ...state, errorText: 'Error!'}))
	.case(actions.showLoader, (state, payload) => ({ ...state, loaderText: payload}))
	.case(actions.hideLoader, state => ({ ...state, loaderText: ''}))

const showAlertEpic: Epic<AnyAction, AnyAction, RootState> = action$ => action$.pipe(
	ofAction(actions.showSuccsessMessage),
    mergeMap(() => of(actions.hideSuccessMessage()).pipe(
        delay(2000))
	)
)
export default {
